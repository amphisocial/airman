 ...
 */
export function stepFlight(p, input, dt) {
  ...
  const auth = clamp((p.speed - MIN_SPEED * 0.55) / (STALL_SPEED - MIN_SPEED * 0.55), 0.22, 1);
  ...
  const rate = p.speed <= CORNER_SPEED
    ? clamp(p.speed / CORNER_SPEED, 0.3, 1)
    : clamp(CORNER_SPEED / p.speed, 0.55, 1);
  ...
  const target = MIN_SPEED + throttle * (MAX_SPEED - MIN_SPEED);
  p.speed += (target - p.speed) * THRUST_K * dt;
  let f = fwdOf(q);
  p.speed -= f.y * CLIMB_DRAG * dt;
  p.speed -= Math.abs(pitch) * auth * TURN_DRAG * dt;
  p.speed = clamp(p.speed, 22, MAX_SPEED * 1.3);

  // Stalled: the nose sags toward the ground whether you like it or not.
  if (p.speed < STALL_SPEED) {
    const droop = (1 - p.speed / STALL_SPEED) * 1.6 * dt;
